...
namespace LFramework
{
    ...
    [AddComponentMenu("LFramework/Fsm")]
    public sealed class FsmComponent : MonoBehaviour, ILFrameworkModule, IFsmManager
    {
        private readonly Dictionary<TypeNamePair, FsmBase> _fsmDict;
        ...
        public FsmComponent()
        {
            _fsmDict = new Dictionary<TypeNamePair, FsmBase>();
        ...
        public void OnUpdate(float elapseSeconds, float realElapseSeconds)
        {
            ...
            foreach (KeyValuePair<TypeNamePair, FsmBase> fsm in _fsmDict)
            {
        ...
        public bool HasFsm(Type ownerType)
        {
            if (ownerType == null)
            {
                throw new LFrameworkException("Owner type is invalid.");
            }
        ...
        {
            int index = 0;
            FsmBase[] results = new FsmBase[_fsmDict.Count];
            foreach (KeyValuePair<TypeNamePair, FsmBase> fsm in _fsmDict)
            {
        ...
        public IFsm<T> CreateFsm<T>(string name, T owner, params FsmState<T>[] states) where T : class
        {
            ...
            if (HasFsm<T>(name))
            {
                throw new LFrameworkException(Utility.Text.Format("Already exist FSM '{0}'.", typeNamePair));
            }

            Fsm<T> fsm = Fsm<T>.Create(name, owner, states);
            _fsmDict.Add(typeNamePair, fsm);
            return fsm;
        }
        ...
        public bool DestroyFsm<T>(IFsm<T> fsm) where T : class
        {
            if (fsm == null)
            {
                throw new LFrameworkException("FSM is invalid.");
            }
        ...
        {
            return _fsmDict.ContainsKey(typeNamePair);
        }

        private FsmBase InternalGetFsm(TypeNamePair typeNamePair)
        {
            FsmBase fsm = null;
            if (_fsmDict.TryGetValue(typeNamePair, out fsm))
            {
                return fsm;
            }
        ...
        private bool InternalDestroyFsm(TypeNamePair typeNamePair)
        {
            ...
            {
                fsm.Shutdown();
                return _fsmDict.Remove(typeNamePair);
            }
