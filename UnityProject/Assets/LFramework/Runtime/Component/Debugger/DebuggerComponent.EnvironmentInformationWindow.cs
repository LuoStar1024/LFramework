...
namespace LFramework
{
    public sealed partial class DebuggerComponent
    {
        ...
        {
            private IBaseManager _baseComponent = null;
            private IResourceManager _resourceComponent = null;

            public override void Initialize(params object[] args)
            {
                _baseComponent = LFrameworkEntry.GetModule<IBaseManager>();
                ...
                _resourceComponent = LFrameworkEntry.GetModule<IResourceManager>();
...
#endif
                    // TODO
                    // DrawItem("LFramework Version", Version.LFrameworkVersion);
                    // DrawItem("Game Version",
                    //     Utility.Text.Format("{0} ({1})", Version.GameVersion, Version.InternalGameVersion));
                    DrawItem("Resource Version",
                        _resourceComponent.ResourceMode == ResourceMode.EditorSimulate
                            ? "Unavailable in editor resource mode"
                            : (string.IsNullOrEmpty(_resourceComponent.ApplicableGameVersion)
                                ? "Unknown"
                                : Utility.Text.Format("{0} ({1})", _resourceComponent.ApplicableGameVersion,
                                    _resourceComponent.InternalResourceVersion)));
