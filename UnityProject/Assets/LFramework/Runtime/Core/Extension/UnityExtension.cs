...
{
    private static readonly List<Transform> CachedTransformList = new List<Transform>();
    ...
    {
        gameObject.GetComponentsInChildren(true, CachedTransformList);
        for (int i = 0; i < CachedTransformList.Count; i++)
        {
            CachedTransformList[i].gameObject.layer = layer;
        }

        CachedTransformList.Clear();
    }
