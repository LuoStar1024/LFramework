...
namespace Launcher
{
    ...
    public class ProcedureInitResources : ProcedureBase
    {
        ...
        protected override void OnUpdate(ProcedureOwner procedureOwner, float elapseSeconds, float realElapseSeconds)
        {
            ...
            if (_resComponent.ResourceMode == ResourceMode.Updatable || _resComponent.ResourceMode == ResourceMode.WebPlayMode)
            {
                // 这里的 PackageVersion 已经是最新远端版本，用于后续下载器与版本记录。
                Log.Debug($"Updated package Version : from {_resComponent.GetPackageVersion()} to {_resComponent.PackageVersion}");
        ...
        private void OnInitResourcesError(ProcedureOwner procedureOwner, string message)
        {
            ...
            LauncherMgr.ShowMessageBox($"初始化资源失败！点击确认重试 \n <color=#FF0000>{message}</color>", MessageShowType.TwoButton,
                LoadStyle.StyleEnum.Style_Retry, () => { _unityWrapperComponent.StartCoroutineWrapper(InitResources(procedureOwner)); }, Application.Quit);
        }
        
        private bool IsNeedUpdate()
        {
            // 非强更且不能联网时，尝试回退到本地已记录版本继续进入游戏。
            if (_configComponent.UpdateConfig.UpdateStyle == UpdateStyle.Optional && !_resComponent.UpdatableWhilePlaying)
            {
                // 获取上次成功记录的版本
