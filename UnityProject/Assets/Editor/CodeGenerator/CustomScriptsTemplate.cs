...
{
    internal class CustomScriptsTemplate
    {
        private const string LauncherScriptTemplate = "Assets/Editor/CodeGenerator/LauncherScriptTemplate.txt";
        private const string GameLogicScriptTemplate = "Assets/Editor/CodeGenerator/GameLogicScriptTemplate.txt";
        private const string GameBattleScriptTemplate = "Assets/Editor/CodeGenerator/GameBattleScriptTemplate.txt";

        [MenuItem("Assets/Create/C# Script Launcher", false, 71)]
        private static void CreateLauncherCustomScript()
        {
            var paths = GetSelectedDirPath("NewLauncherScript");
            if (paths.Count != 0) CreateScriptFile(paths, LauncherScriptTemplate);
            else EditorUtility.DisplayDialog("提示", "请选择文件夹", "确定");
        }

        [MenuItem("Assets/Create/C# Script GameLogic", false, 72)]
        private static void CreateGameLogicCustomScript()
        {
            var paths = GetSelectedDirPath("NewGameLogicScript");
        ...
        {
            var paths = GetSelectedDirPath("NewGameBattleScript");
            if (paths.Count != 0) CreateScriptFile(paths, GameBattleScriptTemplate);
            else EditorUtility.DisplayDialog("提示", "请选择文件夹", "确定");
        }
    ...
    internal class NameByEnterOrUnfocus : EndNameEditAction
    {
        ...
        private static Object CreateScript(string pathName, string resourceFile)
        {
            ...
            //     System.DateTime.Now.Year, System.DateTime.Now.Month, System.DateTime.Now.Day, System.DateTime.Now.Hour,
