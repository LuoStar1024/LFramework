...
namespace LFramework
{
    ...
    public struct LFrameworkLinkedListRange<T> : IEnumerable<T>, IEnumerable
    {
        ...
        public LFrameworkLinkedListRange(LinkedListNode<T> first, LinkedListNode<T> terminal)
        {
            if (first == null || terminal == null || first == terminal)
            {
                throw new LFrameworkException("Range is invalid.");
            }
        ...
        {
            private readonly LFrameworkLinkedListRange<T> m_LFrameworkLinkedListRange;
            private LinkedListNode<T> m_Current;
            private T m_CurrentValue;

            internal Enumerator(LFrameworkLinkedListRange<T> range)
            {
                if (!range.IsValid)
                {
                    throw new LFrameworkException("Range is invalid.");
                ...
                m_LFrameworkLinkedListRange = range;
                m_Current = m_LFrameworkLinkedListRange._first;
            ...
            {
                if (m_Current == null || m_Current == m_LFrameworkLinkedListRange._terminal)
                {
                    return false;
                }
