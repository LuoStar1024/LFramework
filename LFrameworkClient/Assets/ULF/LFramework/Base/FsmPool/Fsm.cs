...
namespace LFramework
{
    ...
    internal sealed class Fsm<T> : FsmBase, IReference, IFsm<T> where T : class
    {
        ...
        private Dictionary<string, Variable> _dataDict;
        ...
        public static Fsm<T> Create(string name, T owner, params FsmState<T>[] states)
        {
            if (owner == null)
            {
                throw new LFrameworkException("FSM owner is invalid.");
            }

            if (states == null || states.Length < 1)
            {
                throw new LFrameworkException("FSM states is invalid.");
            }
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                if (fsm._stateDict.ContainsKey(stateType))
                {
                    throw new LFrameworkException(Utility.Text.Format("FSM '{0}' state '{1}' is already exist.",
                        new TypeNamePair(typeof(T), name), stateType.FullName));
                }

                fsm._stateDict.Add(stateType, state);
                state.OnInit(fsm);
            }
        ...
        public void Clear()
        {
            ...
            foreach (KeyValuePair<Type, FsmState<T>> state in _stateDict)
            {
            ...
            if (_dataDict != null)
            {
                foreach (KeyValuePair<string, Variable> data in _dataDict)
                {
                    if (data.Value == null)
                    {
                        continue;
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            if (IsRunning)
            {
                throw new LFrameworkException("FSM is running, can not start again.");
            }
        ...
        public void Start(Type stateType)
        {
            ...
            if (stateType == null)
            {
                throw new LFrameworkException("State type is invalid.");
            }

            if (!typeof(FsmState<T>).IsAssignableFrom(stateType))
            {
                throw new LFrameworkException(Utility.Text.Format("State type '{0}' is invalid.", stateType.FullName));
            }
        ...
        public TState GetState<TState>() where TState : FsmState<T>
        {
            ...
            if (_stateDict.TryGetValue(typeof(TState), out state))
            {
                return (TState)state;
            }
        ...
        {
            int index = 0;
            FsmState<T>[] results = new FsmState<T>[_stateDict.Count];
            foreach (KeyValuePair<Type, FsmState<T>> state in _stateDict)
            {
        ...
        public bool HasData(string name)
        {
            ...
            {
                throw new LFrameworkException("Data name is invalid.");
            }
        ...
        public Variable GetData(string name)
        {
            ...
            Variable data = null;
            if (_dataDict.TryGetValue(name, out data))
            {
                return data;
            }
        ...
        public void SetData(string name, Variable data)
        {
            ...
            if (_dataDict == null)
            {
                _dataDict = new Dictionary<string, Variable>(StringComparer.Ordinal);
            }
        ...
        internal void ChangeState(Type stateType)
        {
            if (_currentState == null)
            {
                throw new LFrameworkException("Current state is invalid.");
            }
