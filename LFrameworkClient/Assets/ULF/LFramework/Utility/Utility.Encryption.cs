    ...
    public static partial class Utility
    {
        ...
        public static class Encryption
        {
            ...
            public static void GetSelfXorBytes(byte[] bytes, int startIndex, int length, byte[] code)
            {
                ...
                if (code == null)
                {
                    throw new LFrameworkException("Code is invalid.");
                }

                int codeLength = code.Length;
                if (codeLength <= 0)
                {
                    throw new LFrameworkException("Code length is invalid.");
                }

                if (startIndex < 0 || length < 0 || startIndex + length > bytes.Length)
                {
                    throw new LFrameworkException("Start index or length is invalid.");
                }
